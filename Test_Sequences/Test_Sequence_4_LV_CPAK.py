...
log=open("Test_4_LV_CPAK_"+ M_VN + '_' + M_SN + time.strftime('%H_%M_%d_%m_%Y.txt'),"w")
head="Multimeter,TIMESTAMP" + '\n'
log.write(head)
...
for i in range (0,120): # minutes of test, test duration in minutes
	print "Minute n: "+str(i)+'\n'
	for k in range (1,60):
		#poll=module.poller()
		volt_mm=mm.get_voltage()
		timestamp=time.strftime('%H_%M_%S')
		log.write(str(volt_mm)+ ',' + timestamp +'\n')
		time.sleep(0.94)
